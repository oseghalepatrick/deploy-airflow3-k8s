...
def upload_and_run_aws_glue_job():
    ...
    @task
    def upload_directory_to_s3(s3_bucket, local_directory, s3_key_prefix):
        """
        Upload all files from the local directory to S3 in one task.
        """
        import os
        import boto3
        ...
        for root, dirs, files in os.walk(local_directory):
            for file in files:
                ...
                s3_key = os.path.join(s3_key_prefix, os.path.relpath(local_file_path, local_directory))
                
                # Upload each file to S3
                s3.upload_file(local_file_path, s3_bucket, s3_key)
                print(f"Uploaded {local_file_path} to s3://{s3_bucket}/{s3_key}")

    upload_task = upload_directory_to_s3(S3_BUCKET, LOCAL_GLUE_SCRIPT, S3_GLUE_SCRIPT_KEY)
