...
for elem in elements:
    ...
    rows.append(
        (
            ...
            attrib.get("TagName"),
            to_long(attrib.get("Count")),
            to_long(attrib.get("ExcerptPostId")),
            to_long(attrib.get("WikiPostId")),
        )
    )

tags_df = spark.createDataFrame(rows, schema=schema)
tags_df.printSchema()
tags_df.show(5, truncate=True)

temp_view = "tmp_tags"
tags_df.createOrReplaceTempView(temp_view)
