...
for elem in elements:
    ...
    rows.append(
        (
            ...
            attrib.get("UserDisplayName"),
            to_long(attrib.get("UserId")),
        )
    )

comments_df = spark.createDataFrame(rows, schema=schema)
comments_df.printSchema()
comments_df.show(5, truncate=True)

temp_view = "tmp_comments"
comments_df.createOrReplaceTempView(temp_view)
