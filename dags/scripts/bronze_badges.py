...
schema = StructType(
    [
        ...
        StructField("TagBased", StringType(), True),
    ]
)
...
for elem in elements:
    ...
    rows.append(
        (
            ...
            attrib.get("Name"),
            to_timestamp(attrib.get("Date")),
            to_long(attrib.get("Class")),
            attrib.get("TagBased"),
        )
    )

badges_df = spark.createDataFrame(rows, schema=schema)
badges_df.printSchema()
badges_df.show(5, truncate=True)

temp_view = "tmp_badges"
badges_df.createOrReplaceTempView(temp_view)
