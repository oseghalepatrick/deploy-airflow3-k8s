...
for elem in elements:
    ...
    rows.append(
        (
            attrib.get("AboutMe"),
            to_long(attrib.get("AccountId")),
            to_timestamp(attrib.get("CreationDate")),
            attrib.get("DisplayName"),
            to_long(attrib.get("DownVotes")),
            to_long(attrib.get("Id")),
            to_timestamp(attrib.get("LastAccessDate")),
            attrib.get("Location"),
            to_long(attrib.get("Reputation")),
            to_long(attrib.get("UpVotes")),
            to_long(attrib.get("Views")),
            attrib.get("WebsiteUrl"),
        )
    )

users_df = spark.createDataFrame(rows, schema=schema)
users_df.printSchema()
users_df.show(5, truncate=True)

temp_view = "tmp_users"
users_df.createOrReplaceTempView(temp_view)
