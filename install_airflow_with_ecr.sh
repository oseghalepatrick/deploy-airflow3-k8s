...
export ECR_REGISTRY=465002806875.dkr.ecr.eu-west-1.amazonaws.com
export ECR_REPO=my-dags
export NAMESPACE=airflow
export RELEASE_NAME=airflow

# Authenticate with ECR
aws --profile patrick ecr get-login-password --region $REGION \
    | docker login --username AWS --password-stdin $ECR_REGISTRY

# Get the latest image tag from ECR
export IMAGE_TAG=$(
    aws --profile patrick ecr list-images \
